...
async function main() {
  ...
  const serverName = args.name || 'scrape-markdown-tool';
  ...
  if (!serverExists) {
    throw new Error(`Server dist file not found: ${serverDist}. Run "npm run build" in tools/scrape-markdown-tool first.`);
  }
