...
async function main() {
  ...
  const serverName = args.name || 'scrape-pipeline-tool';
  ...
  if (!serverExists) {
    throw new Error(`Server dist file not found: ${serverDist}. Run "npm run build" in tools/scrape-pipeline-tool first.`);
  }
